...
// Load both standard and local env vars
config({ path: '.env' })
...
// Ensure env variables are available (assuming run with --env-file=.env.local)
...
if (!process.env['OPENAI_API_KEY']) {
  console.error('Missing OPENAI_API_KEY. Set it before running or use --env-file=.env.local')
  process.exit(1)
}
...
const articles = [
  ...
  },
  {
    ...
    translations: {
      es: {
        summary:
          'A medida que las organizaciones se vuelven cada vez más globales, la necesidad de líderes que puedan navegar por los matices culturales es crítica. Esta revisión analiza 40 equipos multiculturales de Aprendizaje en Acción.',
      },
      pt: {
        summary:
          'À medida que as organizações se tornam cada vez mais globais, a necessidade de líderes que possam navegar pelas nuances culturais é crítica. Esta revisão abrangente analisa 40 equipes multiculturais.',
      },
      fr: {
        summary:
          'Alors que les organisations deviennent de plus en plus mondiales, le besoin de leaders capables de naviguer dans les nuances culturelles est critique. Cette revue analyse 40 équipes multiculturelles.',
      },
    },
  },
  {
    ...
    published_year: 2025,
    summary:
      'Agile transformations often fail when teams adopt the ceremonies but not the mindset. This article details a case study of a mid-sized software company that integrated WIAL Action Learning coaches into their Agile sprint retrospectives. The integration led to a 40% improvement in sprint velocity and significantly higher psychological safety scores across engineering pods. Action Learning prevented developers from assigning blame and instead focused them on systemic root causes.',
    key_findings: [
    ...
    translations: {
      es: {
        summary:
          'Las transformaciones ágiles a menudo fracasan cuando los equipos adoptan las ceremonias pero no la mentalidad. Este artículo detalla un estudio de caso sobre la integración de coaches de Aprendizaje en Acción.',
      },
      pt: {
        summary:
          'As transformações ágeis geralmente falham quando as equipes adotam as cerimônias, mas não a mentalidade. Este artigo detalha um estudo de caso da integração de coaches de Action Learning.',
      },
      fr: {
        summary:
          "Les transformations agiles échouent souvent lorsque les équipes adoptent les cérémonies mais pas l'état d'esprit. Cet article détaille une étude de cas sur l'intégration de coachs d'Apprentissage par l'Action.",
      },
    },
  },
  {
    ...
    published_year: 2021,
    summary:
      'Non-profit boards frequently struggle with alignment, leading to operational paralysis. We documented the intervention of an Action Learning coach with a deeply divided environmental NGO board. By strictly enforcing the "questions only" rule during three dedicated session, the board was able to move past personal grievances, uncover a shared strategic vision, and secure a $2M grant that was previously stalled by infighting.',
    key_findings: [
      {
    ...
    translations: {
      es: {
        summary:
          'Las juntas sin fines de lucro a menudo luchan con la alineación. Documentamos la intervención de un coach de Aprendizaje en Acción con una junta directiva de ONG profundamente dividida.',
      },
      pt: {
        summary:
          'Conselhos sem fins lucrativos frequentemente lutam com o alinhamento. Documentamos a intervenção de um coach de Action Learning com um conselho de ONG profundamente dividido.',
      },
      fr: {
        summary:
          "Les conseils d'administration de fondations luttent souvent avec l'alignement. Nous avons documenté l'intervention d'un coach d'Apprentissage par l'Action avec un conseil d'ONG profondément divisé.",
      },
    },
  },
]
...
const webinars = [
  ...
  },
  {
    ...
    description:
      'Elena Rodriguez breaks down why Agile transformations get stuck and how WIAL certified coaches can integrate Action Learning directly into Sprint Retrospectives to boost psychological safety and deliver a 40% jump in sprint velocity. Perfect for Scrum Masters, Agile Coaches, and Engineering Managers.',
...
async function seedAiData() {
  ...
  // 1. Embed Articles - ALREADY DONE
  console.log('Skipping articles (already seeded)')

  // 2. Insert Webinars
  console.log('Inserting webinars...')
  const { error: webError } = await supabase.from('webinars').insert(webinars)

  if (webError) {
    console.error('Error inserting webinars:', webError)
  } else {
    console.log('✅ Successfully seeded 2 upcoming Webinars!')
  }
