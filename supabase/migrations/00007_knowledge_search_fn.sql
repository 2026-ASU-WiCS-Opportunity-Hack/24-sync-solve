...
RETURNS TABLE (
  id uuid, title text, summary text, relevance_tags text[],
  translations jsonb, published_year int, similarity float
)
LANGUAGE plpgsql AS $$
BEGIN
  RETURN QUERY
  SELECT
    ja.id, ja.title, ja.summary, ja.relevance_tags,
    ja.translations, ja.published_year,
