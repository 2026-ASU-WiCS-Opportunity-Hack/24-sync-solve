      ...
      <div className="flex-col items-start px-6 pt-6 pb-0">
        ...
        {article.authors && article.authors.length > 0 && (
          <p className="mt-1 line-clamp-1 text-sm text-gray-500">
            {article.authors.join(', ')} • {article.published_year}
          </p>
        )}
      </div>
      ...
      <div className="flex flex-wrap gap-2 rounded-b-xl px-6 pt-0 pb-6">
        ...
        {article.pdf_url && canDownload && (
          <a
            ...
            className="text-wial-red mt-auto ml-auto block pt-2 text-right text-xs font-semibold hover:underline"
          >
            Read Full Text &rarr;
          </a>
        )}
        {article.pdf_url && !canDownload && (
          <span
            className="mt-auto ml-auto block pt-2 text-right text-xs font-semibold text-gray-400"
