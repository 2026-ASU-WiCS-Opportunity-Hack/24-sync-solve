...
export function ArticleUploadForm() {
  ...
  const [modelProvider, setModelProvider] = useState<AiModelProvider>('claude')
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const result = await uploadArticle(formData, modelProvider)
      if (result.success) {
        toast.success(
          `Article processed with ${result.modelUsed === 'claude' ? 'Claude' : 'GPT-4o'}!`
        )
        router.refresh()
        setTitle('')
        setAuthors('')
      ...
      toast.error(err instanceof Error ? err.message : 'Error processing article')
    } finally {
      setLoading(false)
    }
  }
    ...
    <div className="mx-auto mt-8 max-w-xl overflow-hidden rounded-xl border bg-white shadow-sm">
      <div className="p-8">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label htmlFor="article-title" className="mb-1 block text-sm font-medium">
              Article Title *
            </label>
            <input
              id="article-title"
          ...
          <div>
            <label htmlFor="article-authors" className="mb-1 block text-sm font-medium">
          ...
          <div>
            <label htmlFor="article-year" className="mb-1 block text-sm font-medium">
              Published Year *
            </label>
            <input
              id="article-year"
          ...
          <div>
            <fieldset>
              <legend className="mb-2 block text-sm font-medium">AI Summarization Model</legend>
              <div className="flex gap-4">
                {(['claude', 'gpt-4o'] as AiModelProvider[]).map((m) => (
          ...
          <div className="pt-2">
            <label htmlFor="article-pdf" className="mb-2 block pl-1 text-sm font-medium">
              PDF File *
            </label>
            <input
              id="article-pdf"
