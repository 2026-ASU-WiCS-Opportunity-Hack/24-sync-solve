...
import { WebinarPromoterModal } from '@/components/knowledge/WebinarPromoterModal'
...
export function WebinarCard({ webinar, canPromote = false }: WebinarCardProps) {
      ...
      <div className="flex-grow p-6">
        <div className="text-wial-red mb-4 inline-flex items-center gap-1.5 rounded-full bg-red-50 px-2.5 py-1 text-xs font-semibold tracking-wider uppercase">
          {webinar.recording_url ? (
            <>
              <Video className="h-3.5 w-3.5" aria-hidden="true" /> Recording Available
            </>
          ) : (
            <>
              <CalendarDays className="h-3.5 w-3.5" aria-hidden="true" /> Upcoming
            </>
          )}
        </div>
        ...
        <div className="mb-4 flex items-center gap-2 text-sm font-medium text-gray-500">
          {webinar.presenter && (
            <span className="flex items-center gap-1">
              <User className="h-4 w-4" aria-hidden="true" /> {webinar.presenter}
            </span>
          )}
          {webinar.presenter && webinar.scheduled_at && (
