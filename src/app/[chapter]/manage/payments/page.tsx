...
import { getChapterPayments } from '@/features/payments/queries/getPayments'
...
interface ChapterPaymentsPageProps {
...
export default async function ChapterPaymentsPage({ params }: ChapterPaymentsPageProps) {
  ...
  // Permission check: must have payment:view_chapter for this chapter
  const ctx = await getPermissionContext()
  if (!ctx) redirect(`/login?redirect=/${chapterSlug}/manage/payments`)
  if (ctx.isSuspended) redirect('/suspended')

  const canView =
    ctx.globalRole === 'super_admin' || canPerformInChapter(ctx, chapter.id, 'payment:view_chapter')

  if (!canView) redirect('/unauthorized')

  const { items: payments, total, totalCollected } = await getChapterPayments(supabase, chapter.id)
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">
            {total} payment record{total !== 1 ? 's' : ''} for {chapter.name}.
          </p>
        </div>
        {totalCollected > 0 && (
      ...
      {payments.length === 0 ? (
        ...
        <div className="overflow-x-auto rounded-2xl border border-gray-200 bg-white">
          <table className="w-full text-sm" aria-label="Chapter payments list">
            <thead>
